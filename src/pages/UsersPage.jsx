...
const UsersPage = () => {
  ...
  return <div className="page"><h1 className="page__title">Все пользователи</h1>{error && <p className="page__hint">{error}</p>}
    <div className="admin-grid" style={{gridTemplateColumns:'2fr 1fr 1fr'}}>
      ...
      <div className="admin-field"><label>Роль</label><select value={role} onChange={(e)=>setRole(e.target.value)}><option value="">Все</option><option value="student">student</option><option value="teacher">teacher</option><option value="admin">admin</option></select></div>
    ...
    <div className="admin-table-wrapper"><table className="table admin-table"><thead><tr><th>Логин</th><th>ФИО</th><th>Роль</th><th>Группа</th><th></th></tr></thead><tbody>{filtered.map((u)=><tr key={u.id}><td>{u.username}</td><td>{u.full_name}</td><td>{u.role}</td><td>{u.group}</td><td>{u.id!==user?.id && <button className="btn-secondary" onClick={()=>onDelete(u.id)}>Удалить</button>}</td></tr>)}</tbody></table></div>
  </div>;
};
