...
const Dashboard = () => {
  ...
  if (user?.role === 'student') {
    return <div className="page"><h1 className="page__title">Главная</h1>{error && <p className="page__hint">{error}</p>}
      ...
      <div className="admin-card" style={{marginTop:16}}><h2 className="admin-card__title">Последний пройденный тест</h2>{data.last_result ? <p>{data.last_result.test_title} · {Number(data.last_result.score_percent).toFixed(1)}% · {data.last_result.passed ? 'Зачёт' : 'Незачёт'}</p> : <p className="page__hint">Завершённых тестов пока нет</p>}</div>
      <div className="admin-card" style={{marginTop:16}}><h2 className="admin-card__title">Тесты к прохождению</h2>{(data.pending_tests||[]).length===0 ? <p className="page__hint">Нет тестов к прохождению</p> : <div className="admin-table-wrapper"><table className="table"><thead><tr><th>Тест</th><th>Уровень</th><th>Длительность</th><th></th></tr></thead><tbody>{data.pending_tests.map((t)=><tr key={t.id}><td>{t.title}</td><td>{t.level}</td><td>{t.duration_minutes} мин</td><td><Link className="btn-primary" to={`/tests/${t.id}`}>Начать</Link></td></tr>)}</tbody></table></div>}</div>
    </div>;
  }
